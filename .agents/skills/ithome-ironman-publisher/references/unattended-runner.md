...
The adapter persists one hidden, non-overwritable per-Day click receipt in the configured event directory and records `publishClickCount = 1` immediately before dispatching the one allowed click, not after Playwright acknowledges it. An existing receipt blocks another DOM click even after a process restart. A receipt followed by a lost browser acknowledgement is therefore conservative evidence requiring read-only recovery; it is never safe to retry automatically. After dispatch, the adapter reads a fresh DOM snapshot to distinguish an accepted transition, a still-visible confirmation layer, a server error, or an unknown pending state. It never clicks a newly exposed confirmation control. It then performs at most six read-only verification attempts, five seconds apart, and requires both an exact public article match and disappearance of the exact draft before returning `verified`.
