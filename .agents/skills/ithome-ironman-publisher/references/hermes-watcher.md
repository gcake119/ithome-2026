...
- A check prefers the verified publish event. If that event is absent, such as after a manual publication, it reads only the scheduled source file's bounded frontmatter metadata; it never reads or emits the article body. It then follows the verified public series page to its highest pagination page. If the public series page is unavailable, it falls back to the official series RSS. It requires one unique title match after ignoring quotation-mark characters that iThome may remove, requires every other title character to match, requires that article to be the latest entry, then fetches the public article and requires the scheduled publication date and exact canonical URL.
...
- The first verified result for a scheduled Day emits one `public_article_verified` notification containing the public article URL. Verification delivery is deduplicated by Day and date across both checkpoints, so the later checkpoint stays silent. Missing, ambiguous, non-latest, title-mismatched, or canonical-mismatched public evidence produces a publication reminder. Missing publisher-event evidence alone is not treated as a publication failure when the exact public article can be independently verified.
