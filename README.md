...
晚間檢查會找到系列的最後一頁與最後一篇文章；如果系列頁暫時不可讀，則改讀官方系列 RSS。接著核對文章標題、發布日期、網址，以及文章裡的個人連載網站連結。全部正確就不傳訊息；有缺漏才通知你。
...
晚間檢查請依序確認：
1. 優先讀取今天發布成功後留下的公開文章資料；如果是人工發布而沒有這筆事件，只讀取當日來源檔的 frontmatter 中繼資料，不讀取文章正文。
2. 使用 Day 1 已確認的正式系列網址，不要猜網址。
3. 找到系列最後一頁與最後一篇文章。
4. 以唯一且完整相同的標題定位文章；iThome 若移除引號字元可忽略該差異，其餘字元仍須相同。接著核對文章網址與發布日期；若正規化後的同名結果不只一筆，停止判定並通知人工確認。
5. 打開公開文章，核對個人連載網站連結。
